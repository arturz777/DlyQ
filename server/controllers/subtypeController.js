const { SubType, Translation } = require("../models/models"); // Импорт модели SubType
  ...
  async getAll(req, res) {
    try {
      ...
      translations.forEach((t) => {
        ...
        if (!translationMap[subtypeId]) translationMap[subtypeId] = { name: {} };
      ...
      if (translations) {
        ...
        if (typeof translations === "string") {
            try {
                ...
                return res.status(400).json({ message: "Ошибка парсинга translations." });
            }
        } else {
