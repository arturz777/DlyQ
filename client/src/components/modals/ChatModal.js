      ...
      <div className={styles.chatContainer} onClick={(e) => e.stopPropagation()}>
        {!isAuthed ? (
          <div className={styles.guestBox}>
            <div className={styles.guestTitle}>{t("you are not registered", { ns: "auth" })}</div>
            <div className={styles.guestText}>
              {t("log in or sign up to contact support", { ns: "auth" })}
            </div>

            <div className={styles.guestActions}>
              <Link to="/login" className={styles.guestBtnPrimary} onClick={closeChat}>
                {t("login", { ns: "auth" })}
              </Link>
              <Link to="/registration" className={styles.guestBtn} onClick={closeChat}>
                {t("registration", { ns: "auth" })}
              </Link>
              <button className={styles.guestBtn} onClick={closeChat} type="button">
                {t("close", { ns: "auth" })}
              </button>
            </div>
          </div>
